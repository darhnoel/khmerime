#!/usr/bin/env python3
"""Draft, undo, and save state for the KhmerIME lexicon chunk editor.

EditorState is a deep module: a small mutation interface (the ``api_*``
methods) over the full complexity of chunk drafts, undo/redo, validation,
and the save/build/check cycle. It accepts its ``chunks_dir``/``runtime_path``
so it can be driven against a fixture in tests.
"""
...
class EditorState:
    ...
    def api_delete_rows(self, payload: dict[str, object]) -> dict[str, object]:
        row_ids = [str(value) for value in payload.get("row_ids", [])]
        if not row_ids:
            raise EditorError("no rows selected")
        located = [self.find_row(row_id) for row_id in row_ids]
        self.push_undo()
        touched: set[str] = set()
        for draft, _, row in located:
            if row in draft.rows:
                draft.rows.remove(row)
                touched.add(draft.name)
        for name in touched:
            self.mark_dirty(name)
        return {"deleted": len(row_ids), "meta": self.api_meta()}
